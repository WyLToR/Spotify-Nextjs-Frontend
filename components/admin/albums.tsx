...
export default function AlbumsDetail() {
    ...
    const albumFormik = useFormik({
        onSubmit: (formData: Album) => {
            update({formData, artistId: updateAlbum?.artistId, albumId: updateAlbum?.id})
            albumFormik.resetForm()
            const isCreateSuccess = (updateAlbumSuccess && !updateAlbumError && !updateAlbumData?.error);
            if (isCreateSuccess) {
                setTimeout(() => {
                    setAlbumModal(false);
                    setUpdateAlbum(null)
    ...
    const songFormik = useFormik({
        onSubmit: (formData: any) => {
            ...
            const isCreateSuccess = (createSongError && !createSongSuccess && !createSongData?.error);
            if (isCreateSuccess) {
                setTimeout(() => {
                    setSongModal(false);
                    setUpdateAlbum(null)
