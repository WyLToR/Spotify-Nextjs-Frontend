...
export default function ArtistsDetail() {
    ...
    const artistFormik = useFormik({
        onSubmit: (formData: Artist) => {
            if (updateArtist) {
                update({formData, artistId: updateArtist.id});
            } else {
                createArtist(formData)
            }
            artistFormik.resetForm()
            console.log(createArtistData)
            const isCreateSuccess = (createArtistSuccess && !createArtistError && !createArtistData?.error) || (updateSuccess && !updateError && !updateData?.error);
            if (isCreateSuccess) {
                setTimeout(() => {
    ...
    const albumFormik = useFormik({
        onSubmit: (formData: Album) => {
            createAlbum({formData, artistId: updateArtist?.id})
            albumFormik.resetForm()
            const isCreateSuccess = (createAlbumSuccess && !createAlbumError && !createAlbumData?.error);
            if (isCreateSuccess) {
                setTimeout(() => {
                    setAlbumModal(false);
                    setUpdateArtist(null)
        ...
        <>
            <div
                ...
                <div className="flex items-center gap-2 p-5">
                    <p>Search</p>
                    <input
                        className="border-red-300 text-red-900 dark:text-red text-md rounded-lg focus:ring-red-500 focus:border-red-500 block  dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white"
                    />
                    <button
